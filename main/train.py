...
import os
import sys
file_dir = os.path.dirname("/content/drive/MyDrive/ColabNotebooks")
...
def cfg(cross_val_split, eval_params, dataset_params, graph_model_params, data_splits):
    ...
    data_splits['train'] = ['mot17_train']
    data_splits['val'] = ['split_1_val']
...
def main(_config, _run):
    ...
    trainer = Trainer(gpus=1,
                      callbacks=[MOTMetricsLogger(compute_oracle_results = _config['eval_params']
                      ['normalize_mot_metrics'],
                      compute_pred_oracles=_config['eval_params']
                      ['compute_pred_oracles']), ckpt_callback],
                      weights_summary = None,
                      checkpoint_callback=False,
                      max_epochs=_config['train_params']['num_epochs'],
                      val_percent_check = _config['eval_params']
                      ['val_percent_check'],
                      check_val_every_n_epoch=_config['eval_params']
                      ['check_val_every_n_epoch'],
                      nb_sanity_val_steps=0,
                      logger =logger,
                      default_save_path=osp.join(OUTPUT_PATH, 'experiments', run_str),
                      accumulate_grad_batches=_config['train_params']
