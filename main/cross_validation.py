...
def main(eval_params, run_id):
    ...
    per_iter_metrics, best_epoch, best_row, best_metric_val = 
    evaluator.evaluate(cols_to_norm=eval_params['mot_metrics_to_norm'],
    best_method_metric = eval_params['best_method_criteria'])


    # Log the overall results obtained
    print( f"Best Metrics where obtained at epoch {best_epoch}, with 
    {eval_params['best_method_criteria']} = {best_metric_val:.3f}")
